...
/**
 * Service to handle Mock Tests CSV import.
 * CSV Format: Enrollment No., Semester Number, Test Name, Marks
 * Flow: enrollment_no → student_id → academic_semesters.id → mock_tests insert
 */
class MockCsvService {
    async processUpload(fileBuffer, fileName, userId) {
        ...
        const [logResult] = await db.query(
            ...
            [fileName, safeUserId, rows.length, 'pending', 'mock_tests']
        );
    ...
    async validateRow(row) {
        ...
        if (!row['Test Name']) errors.push('Test Name is required');
    ...
    async commitImport(batchId, preview) {
        ...
        for (const item of validRows) {
            ...
            try {
                await db.query(`
                    INSERT INTO mock_tests (semester_id, test_name, marks)
                    VALUES (?, ?, ?)
                    ON DUPLICATE KEY UPDATE marks = VALUES(marks)
                `, [item.semester_id, row['Test Name'], parseFloat(row['Marks'])]);
                successCount++;
            } catch (err) {
                failCount++;
    ...
    async getHistory() {
        const [rows] = await db.query(
            'SELECT * FROM import_logs WHERE module_type = "mock_tests" ORDER BY created_at DESC LIMIT 50'
        );
        return rows;
    }
}

module.exports = new MockCsvService();
