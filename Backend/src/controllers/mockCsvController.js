const mockCsvService = require('../services/mockCsvService');

const uploadFile = async (req, res) => {
    try {
        if (!req.file) return res.status(400).json({ error: 'No file uploaded' });
        const result = await mockCsvService.processUpload(
            req.file.buffer,
        ...
        console.error('Mock CSV upload error:', error);
...
const confirmImport = async (req, res) => {
    try {
        ...
        const result = await mockCsvService.commitImport(batchId, preview);
        res.status(200).json({ message: 'Import completed', ...result });
    } catch (error) {
        console.error('Mock CSV confirm error:', error);
        res.status(500).json({ error: error.message || 'Internal server error' });
    }
};
...
const getImportHistory = async (req, res) => {
    try {
        const history = await mockCsvService.getHistory();
