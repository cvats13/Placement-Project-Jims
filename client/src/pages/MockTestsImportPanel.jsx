...
import { Upload, CheckCircle, AlertCircle, RefreshCw, ArrowRight, ClipboardList, History } from 'lucide-react';
import { toast } from 'sonner';
import mockCsvService from '../services/mockCsvService';

export function MockTestsImportPanel() {
  ...
  const handleUpload = async () => {
    ...
    try {
      const result = await mockCsvService.uploadFile(file);
  ...
  const handleConfirm = async () => {
    ...
    setIsProcessing(true);
    try {
      const result = await mockCsvService.confirmImport(uploadResult.batchId, uploadResult.preview);
      toast.success(`✅ Imported ${result.successCount} mock test entries. ${result.failCount} failed.`);
  ...
  const fetchHistory = async () => {
    setIsProcessing(true);
    try { const data = await mockCsvService.getImportHistory(); setImportHistory(data); }
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Mock Tests Import</h2>
          <p className="text-sm text-gray-500">Upload mock test scores linked to student semesters</p>
        </div>
      </div>
      ...
      <div className="flex gap-2 border-b border-gray-200">
        {[{ id: 'upload', label: 'Upload CSV', icon: Upload }, { id: 'history', label: 'History', icon: History }].map(({ id, label, icon: Icon }) => (
          <button key={id} onClick={() => { setTab(id); if (id === 'history') fetchHistory(); }}
            className={`flex items-center gap-2 px-4 py-2.5 text-sm font-medium border-b-2 transition-colors -mb-px ${tab === id ? 'border-violet-600 text-violet-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
      ...
      {tab === 'upload' && step === 'upload' && (
        <Card className="border-2 border-violet-100 shadow-xl">
          <CardHeader className="bg-violet-50 border-b border-violet-100">
            <CardTitle className="text-xl text-violet-900">Upload Mock Tests CSV</CardTitle>
            <p className="text-sm text-violet-600 font-mono mt-1">
              Required columns: Enrollment No., Semester Number, Test Name, Marks
            </p>
          </CardHeader>
          <CardContent className="p-8 space-y-6">
            <div className="border-2 border-dashed border-violet-200 rounded-2xl bg-white p-12 text-center hover:border-violet-400 transition-all group cursor-pointer">
              <input type="file" id="mock-file" className="hidden" onChange={handleFileChange} accept=".csv,.xlsx" />
              <label htmlFor="mock-file" className="cursor-pointer">
                <div className="w-16 h-16 bg-violet-50 rounded-full flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-transform">
            ...
            <div className="flex justify-center">
              <Button onClick={handleUpload} disabled={!file || isProcessing} size="lg" className="bg-violet-600 hover:bg-violet-700 h-12 px-10 rounded-xl">
        ...
        <div className="space-y-6 animate-in fade-in duration-300">
          ...
          {invalidRows.length > 0 && (
            <Card className="border-red-200 bg-red-50/50">
              ...
              <CardContent>
                <Table>
                  <TableHeader><TableRow><TableHead>Row</TableHead><TableHead>Enrollment No.</TableHead><TableHead>Semester</TableHead><TableHead>Test Name</TableHead><TableHead>Issue</TableHead></TableRow></TableHeader>
                  <TableBody>
                    {invalidRows.slice(0, 10).map((r, i) => (
                      <TableRow key={i}>
                        ...
                        <TableCell>{r.data['Test Name'] || '—'}</TableCell>
              ...
              <div className="max-h-80 overflow-y-auto">
                <Table>
                  <TableHeader className="bg-gray-50 sticky top-0">
                    <TableRow><TableHead>Enrollment No.</TableHead><TableHead>Semester</TableHead><TableHead>Test Name</TableHead><TableHead>Marks</TableHead></TableRow>
                  </TableHeader>
                  <TableBody>
                    {validRows.length > 0 ? validRows.slice(0, 15).map((r, i) => (
                      <TableRow key={i}>
                        ...
                        <TableCell>{r.data['Test Name']}</TableCell>
                        <TableCell className="font-bold text-violet-600">{r.data['Marks']}</TableCell>
                      </TableRow>
                    )) : (
      ...
      {tab === 'history' && (
        <Card>
          <CardHeader className="border-b"><CardTitle className="text-lg">Mock Tests Import History</CardTitle></CardHeader>
          <CardContent className="p-0">
            <Table>
